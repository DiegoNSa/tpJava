	...
	public static ArrayList<Pizza> retrievePizzas() {
		...
		try {
			...
			while(myResult.next()) {
				...
				Pizza newPizza = new Pizza(index,codePizza,nomPizza,prixPizza,categoriePizza);
		...
		try {
			...
			myStatement = connection
					.prepareStatement("INSERT INTO pizzas"
							+ " VALUES(?,?,?,?);");
			myStatement.setString(1, newPizza.code);
			myStatement.setString(2, newPizza.libelle);
			myStatement.setDouble(3, newPizza.prix);
			myStatement.setString(4, newPizza.category.getDescription());
